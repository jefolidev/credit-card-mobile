...
export const cardsServices = {
  getCards: async (): Promise<ResponseGetAllCardsUser> => {
    const { data } = await api.get('/buyer/cards')
    return data
  },

  authCard: async (body: AuthCardDTO): Promise<ResponseAuthCard> => {
...
*/
  ...
  getBillingsDetailsCard: async (
    ...
    const { data } = await api.get(
      `/buyer/card/billings/${billingId}/transactions`
    )
    return data
  },

  getSellsCard: async (
    query: QuerySearchSellsCardDTO
  ): Promise<ResponseGetBillingDetails> => {
    const { data } = await api.get('/buyer/card/transactions', {
      params: query,
    })
    return data
  },

  changePasswordCard: async (
    body: ChangePasswordDto
  ): Promise<ResponseWithMessageDto> => {
    const { data } = await api.patch('/buyer/card/password', body)
    return data
  },

  blockCard: async (): Promise<ResponseWithMessageDto> => {
  ...
  payQrCode: async (
    id: string,
    body: BodyPayQrCodeDto
  ): Promise<ResponseBuyByQrCodeDto> => {
