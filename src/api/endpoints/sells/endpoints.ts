...
import type { ResponseCreateSells, ResponseGetAllSells, ResponseGetLimit, ResponseGetSell, ResponseSellQrCode } from "./responses.dto";
...
const getSells = async (filters: FiltersGetSellDto): Promise<ResponseGetAllSells> => {
  const {data} = await api.get('/sells', {params: filters});
  return data;
}

const createSell = async (payload: CreateSellDto): Promise<ResponseCreateSells> => {
...
const createQrCode = async (payload: CreateQrCodeSellDto): Promise<ResponseSellQrCode> => {
...
const confirmCancellation = async (sellId: string, body: ConfirmCancellationDto): Promise<ResponseGetSell> => {
